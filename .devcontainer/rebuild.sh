...
if docker compose -p phx_suns_env ps -q >/dev/null 2>&1; then
    docker compose -p phx_suns_env down
...
docker compose -p phx_suns_env build --no-cache datascience
status_ok "DevContainer rebuilt successfully"

# Step 4: Start the container
log "Step 4: Starting DevContainer..."
docker compose -p phx_suns_env up -d
...
if docker compose -p phx_suns_env ps | grep -q "Up"; then
    status_ok "Container is running"
else
    status_error "Container failed to start"
    echo "Check logs with: docker compose -p phx_suns_env logs -f"
    exit 1
fi

# Step 7: Run setup script inside container
log "Step 7: Running setup script inside container..."
CONTAINER_NAME=$(docker compose -p phx_suns_env ps -q datascience)
...
echo "  View logs: docker compose -p phx_suns_env logs -f"
echo "  Enter container: docker exec -it $CONTAINER_NAME bash"
echo "  Stop container: docker compose -p phx_suns_env down"
